...
object GameView {
  ...
  /**
	...
	def update {
		...
		for(line <- (0 until GameEngineOriginal.height)) {
		  for(column <- (0 until GameEngineOriginal.width)) {
		    print(if (GameEngineOriginal.isCellAlive(line, column))  ALIVE_CELL else DEAD_CELL);
		  }
			/**
  ...
  private def makeCellAlive {
	  ...
	  do {
      print("\n Inform the row number (0 - " + (GameEngineOriginal.height - 1) + "): ")
      line = readInt
      
      print("\n Inform the column number (0 - " + (GameEngineOriginal.width - 1) + "): ")
      column= readInt
  ...
  private def validPosition(line: Int, column: Int): Boolean = {
		println(line);
		println(column);
		line >= 0 && line < GameEngineOriginal.height && column >= 0 && column < GameEngineOriginal.width
	}
